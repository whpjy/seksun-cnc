...
OPERATION_DEFINITIONS = [
    ...
    definition("back_live_face_finishing", "背面动力刀具端面精加工", "L32 车铣复合", "接料切断后使用背面动力刀具按端面边界光栅精加工。", "Profile",
               ["solid", "planar_face", "outer_loop"], ["end_mill"], "EM-1",
               [number("stock_allowance_mm", "背面轴向余量", 0.25, minimum=0.01, group="geometry", required=True),
                number("step_over_mm", "光栅步距", 0.55, minimum=0.01, group="cutting"),
                number("axial_clearance_mm", "轴向安全距离", 1.0, minimum=0.1, group="non_cutting"),
                *COMMON_MILLING], "experimental", False),
    ...
    definition("turn_grooving", "车槽", "槽加工", "加工外槽、内槽或端面槽。", "Grooving",
               ...
               [number("groove_width_mm", "槽宽", 2.0, minimum=0.01, group="geometry", required=True), number("peck_depth_mm", "分层切入量", 0.5, minimum=0.01, group="cutting"), enum("groove_strategy", "切槽策略", "plunge_envelope", ["plunge_envelope", "full_radius_contour"]), *COMMON_TURNING], "experimental", False, provider="turning"),
